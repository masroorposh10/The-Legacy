...
function Layout() {
    ...
    useEffect(() => {
        const asyncFunc = async () => {
            if (obituaries2) {
                const res = await fetch("https://j5o623n6hla56pgqclv6h7xbxy0xijdv.lambda-url.ca-central-1.on.aws/",
                    {
                        headers: {
    ...
    const onSubmitForm = async (e) => {
        ...
        return new Promise((resolve, reject) => {
            fetch(
                "https://dhltcv7b4qbol7frvjcki745a40nkqac.lambda-url.ca-central-1.on.aws/",
                {
                    method: "POST",
                    body: data
                }
    ...
    if (isLoading) {
        return <div style={{alignContent:"center", textAlign:"center", paddingTop:"25%"}}><BeatLoader color="#00BFFF" size={20} margin={10} /></div>; 
      }
    return (
        <>
            <header>
            <div id="app-header">
              ...
              <h5 id="app-moto">Obituaries for the G.O.A.T.s</h5>
            </div>
                <div className="newButton">
                    <button type='button' className="addNew" onClick={() => { display(); navigate("/obituaries/create") }}>&#43;  Add New</button>
                </div>
            </header>
            ...
            <footer  style={{textAlign:"center",paddingTop:"98px",color:"purple"}}>@byMasroor</footer>
        </>
